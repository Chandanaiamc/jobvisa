...
<section class="panel">
    <div class="panel-head">
        <div>
            <h1 class="panel__title">Portfolio Builder History</h1>
            <p class="panel__lead"><?= e((string) ($resume['title'] ?? 'Resume')) ?></p>
        </div>
        <a class="btn btn--secondary" href="<?= e(app_url('/jobseeker/resumes/' . (int) $resume['id'] . '/portfolio-builder')) ?>">Back</a>
    </div>
...
<section class="panel">
    ...
    <?php else: ?>
        <div class="table-wrap">
            <table class="table">
                <thead>
                    <tr><th>Headline</th><th>Action</th><th>When</th><th></th></tr>
                </thead>
                <tbody>
                    <?php foreach ($history as $row): ?>
                        <tr>
                            ...
                            <td>
                                <form method="post" action="<?= e(app_url('/jobseeker/resumes/' . (int) $resume['id'] . '/portfolio-builder/history/' . (int) ($row['id'] ?? 0) . '/delete')) ?>" style="display:inline">
                                    <?= csrf_field() ?>
        ...
        <form method="post" action="<?= e(app_url('/jobseeker/resumes/' . (int) $resume['id'] . '/portfolio-builder/history/clear')) ?>" style="margin-top:1rem">
            <?= csrf_field() ?>
                ...
                <tbody>
                    <?php foreach ($deletedHistory as $row): ?>
                        <tr>
                            ...
                            <td>
                                <form method="post" action="<?= e(app_url('/jobseeker/resumes/' . (int) $resume['id'] . '/portfolio-builder/history/' . (int) ($row['id'] ?? 0) . '/restore')) ?>" style="display:inline">
                                    <?= csrf_field() ?>
                                    <button type="submit" class="btn btn--secondary">Restore</button>
                                </form>
                                <form method="post" action="<?= e(app_url('/jobseeker/resumes/' . (int) $resume['id'] . '/portfolio-builder/history/' . (int) ($row['id'] ?? 0) . '/purge')) ?>" style="display:inline">
                                    <?= csrf_field() ?>
