...
final class MockInterviewHistoryRepository extends BaseRepository implements MockInterviewHistoryRepositoryInterface
{
    protected string $table = 'mock_interview_history';
    ...
    public function append(int $resumeId, int $userId, array $payload): int
    {
        ...
        if ($snapshot === false) {
            throw new \RuntimeException('Unable to encode mock interview history JSON.');
        }

        $this->query(
            'INSERT INTO `mock_interview_history`
                (`resume_id`, `user_id`, `session_id`, `action`, `headline`,
                 `overall_score`, `snapshot_json`, `rules_version`)
             VALUES
                (:resume_id, :user_id, :session_id, :action, :headline,
                 :overall_score, :snapshot_json, :rules_version)',
            [
    ...
    public function listByResumeId(int $resumeId, int $limit = 25): array
    {
        ...
        return $this->fetchAll(
            'SELECT `id`, `resume_id`, `user_id`, `session_id`, `action`, `headline`,
                    `overall_score`, `rules_version`, `created_at`
    ...
    public function listDeletedByResumeId(int $resumeId, int $limit = 25): array
    {
        ...
        return $this->fetchAll(
            'SELECT `id`, `resume_id`, `user_id`, `session_id`, `action`, `headline`,
                    `overall_score`, `rules_version`, `created_at`, `deleted_at`
             FROM `mock_interview_history`
    ...
    {
        $row = $this->fetchOne(
            'SELECT `id` FROM `mock_interview_history`
