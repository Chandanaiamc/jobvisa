...
final class PortfolioBuilderHistoryRepository extends BaseRepository implements PortfolioBuilderHistoryRepositoryInterface
{
    protected string $table = 'portfolio_builder_history';
    ...
    public function append(int $resumeId, int $userId, array $payload): int
    {
        ...
        if ($snapshot === false) {
            throw new \RuntimeException('Unable to encode portfolio builder history JSON.');
        }

        $this->query(
            'INSERT INTO `portfolio_builder_history`
                (`resume_id`, `user_id`, `plan_id`, `action`, `headline`,
                 `strength_score`, `project_count`, `snapshot_json`, `rules_version`)
             VALUES
                (:resume_id, :user_id, :plan_id, :action, :headline,
                 :strength_score, :project_count, :snapshot_json, :rules_version)',
            [
    ...
    public function listByResumeId(int $resumeId, int $limit = 25): array
    {
        ...
        return $this->fetchAll(
            'SELECT `id`, `resume_id`, `user_id`, `plan_id`, `action`, `headline`,
                    `strength_score`, `project_count`, `rules_version`, `created_at`
             FROM `portfolio_builder_history`
    ...
    public function listDeletedByResumeId(int $resumeId, int $limit = 25): array
    {
        ...
        return $this->fetchAll(
            'SELECT `id`, `resume_id`, `user_id`, `plan_id`, `action`, `headline`,
                    `strength_score`, `project_count`, `rules_version`, `created_at`, `deleted_at`
             FROM `portfolio_builder_history`
    ...
    {
        $stmt = $this->query(
            'UPDATE `portfolio_builder_history`
    ...
    {
        $row = $this->fetchOne(
            'SELECT `id` FROM `portfolio_builder_history`
    ...
    {
        $stmt = $this->query(
            'DELETE FROM `portfolio_builder_history`
